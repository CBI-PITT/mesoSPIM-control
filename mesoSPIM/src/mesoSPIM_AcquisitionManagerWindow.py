...
class mesoSPIM_AcquisitionManagerWindow(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        loadUi(
            self.parent.package_directory + "/gui/mesoSPIM_AcquisitionManagerWindow.ui",
            self,
        )
        ...
        self.MarkCurrentETLParametersButton.clicked.connect(
        ...
        self.ImageProcessingWizardButton.clicked.connect(
            self.run_image_processing_wizard
        )
    ...
    def move_selected_row_up(self):
        ...
        if row is not None:
            if row > 0:
                self.model.moveRow(
                    QtCore.QModelIndex(), row, QtCore.QModelIndex(), row - 1
                )
    ...
    def set_item_delegates(self):
        ...
        """
        self.delegate_dict = {
            "x_pos": "MarkXPositionDelegate(self)",
        ...
        for key in self.delegate_dict:
            ...
            string_to_execute = (
                "self.table.setItemDelegateForColumn(column_index,"
                + self.delegate_dict[key]
                + ")"
            )
    ...
    def update_acquisition_size_prediction(self):
        bytes_total = self.parent.core.get_required_disk_space(
            self.model.get_acquisition_list()
        )
    ...
    def save_table(self):
        path, _ = QtWidgets.QFileDialog.getSaveFileName(
            None, "Save Table", directory="./acq_table.bin"
        )
        if path:
    ...
    def preview_acquisition(self):
        ...
        if row is not None:
            ...
            if self.PreviewZCheckBox.checkState():
                ...
                self.parent.sig_state_request.emit(
                    {"state": "preview_acquisition_with_z_update"}
                )
            else:
                # print('Checkbox not checked')
                self.parent.sig_state_request.emit(
                    {"state": "preview_acquisition_without_z_update"}
                )
        else:
            if self.model.rowCount() == 1:
                self.set_selected_row(0)
    ...
    def auto_illumination(self, margin_um=500):
        ...
        for row in range(0, self.model.rowCount()):
            ...
            if x_pos <= x_min + margin_um:
                if (
                    "flip_auto_LR_illumination" in self.cfg.ui_options.keys()
                    and self.cfg.ui_options["flip_auto_LR_illumination"]
                ):
                    logger.info(
                        f"Config parameter 'flip_auto_LR_illumination' = True. Illumination of tile {row} will be set to 'Right'."
                    )
                ...
                if (
                    ...
                    logger.info(
                        f"Config parameter 'flip_auto_LR_illumination' = True. Illumination of tile {row} will be set to 'Left'."
                    )
                    self.model.setShutterconfig(row, "Left")
                else:
                    self.model.setShutterconfig(row, "Right")
