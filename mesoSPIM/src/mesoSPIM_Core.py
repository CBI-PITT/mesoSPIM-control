...
from .mesoSPIM_WaveFormGenerator import (
    mesoSPIM_WaveFormGenerator,
    mesoSPIM_DemoWaveFormGenerator,
)
...
class mesoSPIM_Core(QtCore.QObject):
    ...
    sig_update_gui_from_shutter_state = (
        QtCore.pyqtSignal()
    )  # dirty hack to update the shutter state in the GUI
    ...
    sig_stop_aquisition = (
        QtCore.pyqtSignal()
    )  # STOP signal to both Camera and ImageWriter threads
    ...
    sig_polling_stage_position_start, sig_polling_stage_position_stop = (
        QtCore.pyqtSignal(),
        QtCore.pyqtSignal(),
    )
    ...
    def __init__(self, config, parent):
        ...
        self.sig_run_timepoint.connect(
            self.parent.run_timepoint, type=QtCore.Qt.QueuedConnection
        )
        self.parent.sig_state_request.connect(
            self.state_request_handler, type=QtCore.Qt.QueuedConnection
        )
        self.parent.sig_execute_script.connect(
            self.execute_script, type=QtCore.Qt.QueuedConnection
        )
        self.parent.sig_move_relative.connect(
            self.move_relative, type=QtCore.Qt.QueuedConnection
        )
        # self.parent.sig_move_absolute.connect(self.move_absolute, type=QtCore.Qt.QueuedConnection)
        self.parent.sig_zero_axes.connect(
            self.zero_axes, type=QtCore.Qt.QueuedConnection
        )
        self.parent.sig_unzero_axes.connect(
            self.unzero_axes, type=QtCore.Qt.QueuedConnection
        )
        self.parent.sig_stop_movement.connect(
            self.stop_movement, type=QtCore.Qt.QueuedConnection
        )
        self.parent.sig_load_sample.connect(
            self.sig_load_sample.emit, type=QtCore.Qt.QueuedConnection
        )
        self.parent.sig_unload_sample.connect(
            self.sig_unload_sample.emit, type=QtCore.Qt.QueuedConnection
        )
        self.parent.sig_center_sample.connect(
            self.sig_center_sample.emit, type=QtCore.Qt.QueuedConnection
        )
        self.parent.sig_save_etl_config.connect(
            self.sig_save_etl_config.emit, type=QtCore.Qt.QueuedConnection
        )
        self.sig_update_gui_from_shutter_state.connect(
            self.parent.update_GUI_by_shutter_state, type=QtCore.Qt.QueuedConnection
        )
        ...
        self.camera_worker = mesoSPIM_Camera(
            ...
            frame_queue_display=self.frame_queue_display,
        )
        self.camera_worker.moveToThread(self.camera_thread)
        self.camera_worker.sig_update_gui_from_state.connect(
            self.sig_update_gui_from_state.emit
        )
        self.camera_worker.sig_status_message.connect(self.send_status_message_to_gui)
        self.camera_worker.sig_camera_frame.connect(
            self.parent.camera_window.update_image_from_deque
        )
        self.sig_end_image_series.connect(
            self.camera_worker.end_image_series, type=QtCore.Qt.BlockingQueuedConnection
        )
        self.sig_stop_aquisition.connect(
            self.camera_worker.stop, type=QtCore.Qt.QueuedConnection
        )
        ...
        self.sig_write_metadata.connect(
            self.image_writer.write_metadata, type=QtCore.Qt.BlockingQueuedConnection
        )
        self.sig_end_image_series.connect(
            self.image_writer.end_acquisition, type=QtCore.Qt.BlockingQueuedConnection
        )
        self.sig_stop_aquisition.connect(
            self.image_writer.abort_writing, type=QtCore.Qt.QueuedConnection
        )

        self.camera_worker.sig_write_images.connect(
            self.image_writer.write_images, type=QtCore.Qt.QueuedConnection
        )
        ...
        self.sig_polling_stage_position_start.connect(
            self.serial_worker.stage.pos_timer.start
        )
        self.sig_polling_stage_position_stop.connect(
            self.serial_worker.stage.pos_timer.stop
        )
        ...
        self.sig_move_relative_and_wait_until_done.connect(
            lambda sdict: self.serial_worker.move_relative(sdict, wait_until_done=True)
        )
        self.sig_state_request.connect(self.serial_worker.state_request_handler)
        self.sig_state_request_and_wait_until_done.connect(
            lambda sdict: self.serial_worker.state_request_handler(
                sdict, wait_until_done=True
            )
        )
        ...
        self.waveformer.sig_update_gui_from_state.connect(
            self.sig_update_gui_from_state.emit
        )
        # Signals from Core
        self.sig_state_request.connect(self.waveformer.state_request_handler)
        self.sig_state_request_and_wait_until_done.connect(
            self.waveformer.state_request_handler
        )
        ...
        self.shutterswitch = (
            self.cfg.shutterswitch if hasattr(self.cfg, "shutterswitch") else False
        ...
        self.state["camera_display_live_subsampling"] = self.cfg.startup[
            "camera_display_live_subsampling"
        ]
        self.state["camera_display_acquisition_subsampling"] = self.cfg.startup[
        ...
        if (
            "asi" in self.cfg.stage_parameters["stage_type"].lower()
        ):  # TTL option for ASI stages
            assert hasattr(self.cfg, "asi_parameters"), (
                "Config file for ASI Stages must contain 'asi_parameters' dict."
            )
            self.TTL_mode_enabled_in_cfg = self.read_config_parameter(
                "ttl_motion_enabled", self.cfg.asi_parameters
            )
    ...
    def set_zoom(self, zoom, wait_until_done=True, update_etl=True):
        ...
        if "f_objective_exchange" in self.cfg.stage_parameters.keys():
            ...
            self.move_absolute(
                {"f_abs": self.cfg.stage_parameters["f_objective_exchange"]},
    ...
    def set_laser(self, laser, wait_until_done=False, update_etl=True):
        if wait_until_done:
            ...
            if update_etl:
                self.sig_state_request_and_wait_until_done.emit(
                    {"set_etls_according_to_laser": laser}
                )
        else:
    ...
    def move_absolute(self, sdict, wait_until_done=False, use_internal_position=True):
        if wait_until_done:
            ...
            self.serial_worker.move_absolute(
                sdict, wait_until_done=True, use_internal_position=use_internal_position
            )
    ...
    def snap_image_hdr_series(self, acq, laser_blanking=True):
        ...
        try:
            for i, intensity_ratio in enumerate(acq["hdr_intensity_ratios"]):
                ...
                hdr_intensity = min(100, original_intensity * intensity_ratio)
                ...
                if laser_blanking:
                    if i == 0:
                        ...
                        # Brief disable between exposures to avoid bleed-through
                        self.laserenabler.disable_all()
                        time.sleep(0.005)
                        self.laserenabler.enable(laser)
    ...
    def live(self):
        ...
        laser_blanking = (
            False
            if (
                hasattr(self.cfg, "laser_blanking")
                and (self.cfg.laser_blanking in ("stack", "stacks"))
            )
            else True
        )
        self.laserenabler.enable(laser)
    ...
    def start(self, row=None):
        ...
        if nonexisting_folders_list:
            ...
            self.sig_warning.emit(
                f"The acquisition list contains positions {acqusitions_outside_motion_limits} outside the motion limits - stopping!"
            )
            self.sig_finished.emit()
        else:
    ...
    def get_free_disk_space(self, acq_list):
        ...
        if platform.system() == "Windows":
            ...
            ctypes.windll.kernel32.GetDiskFreeSpaceExW(
                ctypes.c_wchar_p(disk_name), None, None, ctypes.pointer(free_bytes)
            )
    ...
    def get_required_disk_space(self, acq_list):
        ...
        total_bytes_required = (
            acq_list.get_image_count() * px_per_image * BYTES_PER_PIXEL
        )
    ...
    def check_motion_limits(self, acq_list):
        ...
        for i in range(len(acq_list)):
            if not (
                self.cfg.stage_parameters["x_min"]
                <= acq_list[i]["x_pos"] - self.serial_worker.stage.int_x_pos_offset
                <= self.cfg.stage_parameters["x_max"]
            ):
                unsafe_list.append(i)
            elif not (
                self.cfg.stage_parameters["y_min"]
                <= acq_list[i]["y_pos"] - self.serial_worker.stage.int_y_pos_offset
                <= self.cfg.stage_parameters["y_max"]
            ):
                unsafe_list.append(i)
            elif not (
                self.cfg.stage_parameters["z_min"]
                <= acq_list[i]["z_start"] - self.serial_worker.stage.int_z_pos_offset
                <= self.cfg.stage_parameters["z_max"]
            ):
                unsafe_list.append(i)
            elif not (
                self.cfg.stage_parameters["z_min"]
                <= acq_list[i]["z_end"] - self.serial_worker.stage.int_z_pos_offset
                <= self.cfg.stage_parameters["z_max"]
            ):
                unsafe_list.append(i)
            else:
    ...
    def close_acquisition_list(self, acq_list):
        ...
        if not self.stopflag:
            ...
            if (
                current_rotation > target_rotation + 0.1
                or current_rotation < target_rotation - 0.1
            ):
            ...
            self.set_laser(
                acq_list[0]["laser"], wait_until_done=False, update_etl=False
            )
            ...
            self.sig_state_request.emit(
                {"etl_l_amplitude": acq_list[0]["etl_l_amplitude"]}
            )
            self.sig_state_request.emit(
                {"etl_r_amplitude": acq_list[0]["etl_r_amplitude"]}
            )
            ...
            time.sleep(
                0.1
            )  # tiny sleep period to allow Main Window indicators to catch up
            self.sig_finished.emit()
    ...
    def preview_acquisition(self, z_update=True):
        ...
        rotationflag = (
            True
            if current_rotation > target_rotation + 0.1
            or current_rotation < target_rotation - 0.1
            else False
        )
    ...
    def prepare_acquisition(self, acq, acq_list):
        ...
        logger.info(
            f"Core: Running Acquisition #{self.acquisition_count} with Filename: {acq['filename']}"
        )
        self.state["selected_row"] = (
            self.acquisition_count
        )  # this make sure that the correct row is selected in the GUI
        self.sig_status_message.emit("Going to start position")
        current_rotation = self.state["position"]["theta_pos"]
        startpoint = acq.get_startpoint()
        target_rotation = startpoint["theta_abs"]
        ...
        for i in range(steps):
            if self.stopflag is True:
                ...
                if acq.get("hdr_enabled", False):
                    ...
                    # Standard single exposure capture path
                    self.snap_image_in_series(laser_blanking)
                ...
                time_remaining = (
                    time_passed
                    / self.image_count
                    * (self.total_image_count - self.image_count)
                )
                ...
                if ((self.image_count + 1) % 5 == 0) or (
                    self.total_image_count % (self.image_count + 1) == 0
                ):
                    self.send_progress(
                        self.acquisition_count,
    ...
    def close_acquisition(self, acq, acq_list):
        self.sig_status_message.emit(
            "Closing Acquisition: Saving data & freeing up memory"
        )
        ...
        self.state["current_framerate"] = acq.get_image_count() / (
            self.image_acq_end_time - self.image_acq_start_time
        )
    ...
    def read_config_parameter(self, key, dictionary):
        ...
        if key not in dictionary.keys():
            message = (
                f"Mandatory parameter {key} not found in dictionary: \n {dictionary} \n"
    ...
    def append_timing_info_to_metadata(self, acq):
        ...
        with open(metadata_path, "a") as file:
            ...
            write_line(
                file,
                "Total time of taking images, s",
                str(round(self.image_acq_end_time - self.image_acq_start_time, 2)),
            )
            write_line(
                file,
                "Total time of stack acquisition, s",
                str(round(self.acq_end_time - self.acq_start_time, 2)),
            )
            write_line(
                file,
                "Frame rate during taking images, img/s:",
                str(round(self.state["current_framerate"], 2)),
            )
            write_line(
                file, "===================== END OF ACQUISITION ======================"
            )
            write_line(file)
    ...
    def run_time_lapse(self, tpoints=1, time_interval_sec=60):
        ...
        QtCore.QTimer.singleShot(
            ...
            lambda: self.run_time_lapse(tpoints, time_interval_sec),
        )
